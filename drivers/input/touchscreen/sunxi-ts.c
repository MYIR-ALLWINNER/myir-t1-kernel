...
#define DISCARD_DATA_SIZE (2)
#define MAX_D_VALUE_X	(150)
#define MAX_D_VALUE_Y	(100)
...
static int do_demounce(int x, int y, int x_tmp, int y_tmp)
{
	if(abs(x - x_tmp) > MAX_D_VALUE_X || abs(y - y_tmp) > MAX_D_VALUE_Y)
		return 1;
	else
		return 0;
}
static irqreturn_t sunxi_ts_irq_handler(int irq, void *dev_id)
{
	...
	if (reg_val & FIFO_DATA_PENDING) {
		...
		if (discard_data >= DISCARD_DATA_SIZE) {
			...
			x = readl(chip->base + TP_DATA);
			y = readl(chip->base + TP_DATA);
			printk("[adbg][%s][%d]: ignore data %d (0x%x, 0x%x)\n", __func__, __LINE__, discard_data, x, y);
			discard_data++;
		}
	}
	...
	spin_unlock_irqrestore(&chip->splock, irqflags);
	return IRQ_HANDLED;
}
...
static void sunxi_ts_hwinit(struct sunxi_ts *chip)
{
	...
	ts_config->tp_sensitive_adjust = 2;
	ts_config->filter_type = 3;

	writel(ADC_FIRST_DLY(0xf) | ADC_FIRST_DLY_MODE(0) | ADC_CLK_DIV(0) |
			FS_DIV(9) | T_ACQ(0xf), chip->base + TP_CTRL0);
	...
	printk("[adbg][%s][%d]: TP_CTRL2 0x%x\n"
		, __func__, __LINE__, TP_SENSITIVE_ADJUST(ts_config->tp_sensitive_adjust) | TP_MODE_SELECT(0));
	writel(TP_SENSITIVE_ADJUST(ts_config->tp_sensitive_adjust) | TP_MODE_SELECT(0),
	       chip->base + TP_CTRL2);


	reg_val = readl(chip->base + TP_CTRL2);
	printk("[adbg][%s][%d]: reg_val 0x%x\n", __func__, __LINE__, reg_val);
	writel(reg_val | PRE_MEA_THRE_CNT(0xffffff), chip->base + TP_CTRL2);
	...
	*/
	reg_val = STYLUS_UP_DEBOUN(0xf) | STYLUS_UP_DEBOUN_EN(1)
